...
@pytest.mark.asyncio
async def test_get_all_busy_couriers(db_session_with_commit: AsyncSession):
    # Создаем контейнер
    container = Container()
    ...
    order_repository: OrderRepository = await container.order_repository()
    ...
    get_all_busy_couriers = await container.get_all_busy_couriers_use_case()
